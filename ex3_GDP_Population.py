...
"""
fig = plt.figure()
box1 = fig.add_subplot(1, 3, 1)
box2 = fig.add_subplot(1, 3, 2)
box3 = fig.add_subplot(1, 3, 3)
box1.boxplot(df.medals)
box2.boxplot(df.GDP)
box2.boxplot(df.Population)
plt.show()
"""
...
# Adj. R-squared:                  0.241
# Prob (F-statistic):           4.69e-08    --------> 유의한 모델
# slope_GDP = 0.0142 : slope_Population = 8.611e-07 : bias(intercept) = -33.1082
print('결정계수(설명력): ', result.rsquared) 
# 결정계수(설명력):  0.25435103510041146 -----> 설명력이 적다.
...
df.Population = int(input('Population를 입력하세요: '))
pred = result.predict(pd.DataFrame({'GDP':df.GDP,'Population':df.Population}))
...
# 더빈왓슨 값으로 확인: Durbin-Watson:                   0.763
...
print(variance_inflation_factor(model.exog, 2)) # 1.008295250855819 < 10
